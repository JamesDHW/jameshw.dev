...
const MobileNav: FC<MobileNavProps> = ({ open, setOpen }) => {
  return (
    <div
      className={`md:hidden fixed top-0 left-0 z-[60] h-screen w-screen transform ${open ? "-translate-x-0" : "-translate-x-full"
        } transition-transform duration-100 ease-in-out bg-gray-50/95 dark:bg-gray-900/95 drop-shadow-md overscroll-contain`}
    >
      <div className="relative h-full w-full">
        <button
          aria-label="Close menu"
          className="absolute top-5 right-8 z-10 w-10 h-10 bg-gray-200 rounded-lg dark:bg-gray-600 flex items-center justify-center hover:ring-2 ring-gray-300 transition-all"
          onClick={() => setOpen(false)}
          type="button"
        >
          <CrossIcon className="h-5 w-5 text-gray-900 dark:text-gray-100" />
        </button>
